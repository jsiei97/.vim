...
function! FileNameToClass()
    let filename=expand("%:t")
    let _name=substitute(filename,'\.h','',"g")
    return _name
endfunction
...
let g:template = {}
let g:template['c'] = {}
" CJ Let's try a doxygen thing...
let g:template['c']['doxy'] = "/**\<cr> ".g:rs."description here".g:re."\<cr>\<cr>@param[in,out] ".g:rs."varname description".g:re."\<cr>@return ".g:rs."description".g:re."\<cr>/"


let g:template['c']['doxyfile'] = "/**\<cr>@file \<c-r>=expand(\"%:t\") \<cr>\<cr>@author Johan Simonsson  \<cr>@brief ".g:rs."brief description".g:re."\<cr>/"

let g:template['c']['doxyheine'] = "/***********************************************************\<cr>\<cr>      HEINE DDS\<cr>\<cr>".repeat("\<left>",7)."**********************************************************\<cr> *\<cr>Copyright c 2010 Heine Optotechnik GmbH & Co. KG.\<cr><info.software@heine.com>\<cr>\<cr>/"


let g:template['c']['mark'] = "/// @warning CJ MARK (\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>) "


let g:template['c']['todo'] = "/// @todo ".g:rs."...".g:re.""
let g:template['c']['bug']  = "/// @bug ".g:rs."Tell med about it...".g:re.""
let g:template['c']['warn'] = "/// @warning ".g:rs."What is dangerus?".g:re.""
...
let g:template['c']['ff'] = "#ifndef  __\<c-r>=GetFileName()\<cr>\<CR>#define  __\<c-r>=GetFileName()\<cr>".
            \repeat("\<cr>",3)."#endif  // __\<c-r>=GetFileName()\<cr> ".repeat("\<up>",3)
...
let g:template['cpp']['class'] = "class \<c-r>=FileNameToClass()\<cr> \<cr>{\<cr> private: \<cr>public: \<cr>\<cr>};"


" ---------------------------------------------
" php templates
let g:template['php'] = g:template['c']
let g:template['php']['php'] = "\<?php\<cr>".g:rs."...".g:re."\<cr>?>"
"let g:template['php']['class'] = "class ".g:rs."name".g:re."\<cr>{\<cr>function __construct(){}\<cr>\<cr>function __destruct(){}\<cr>\<cr>}"
"let g:template['php']['function'] = "function ".g:rs."name".g:re."(".g:rs."args".g:re.")\<cr>{\<cr>".g:rs."...".g:re."\<cr>}"
"let g:template['php']['this'] = "$this->".g:rs."...".g:re

" php doxygen header
let g:template['php']['doxy'] = "/**\<cr>".g:rs."description here".g:re."\<cr>\<cr>@param ".g:rs."varname description".g:re."\<cr>@return ".g:rs."description".g:re."\<cr>/"
...
let g:template['tex'] = {}
let g:template['tex']['date'] = "\<c-r>=strftime(\"%Y-%m-%d %a %H:%M\")\<cr>"

let g:template['tex']['h1'] = "\\section{".g:rs."...".g:re."}"
let g:template['tex']['h2'] = "\\subsection{".g:rs."...".g:re."}"
let g:template['tex']['h3'] = "\\subsubsection{".g:rs."...".g:re."}"
let g:template['tex']['h4'] = "\\paragraph{".g:rs."...".g:re."}"
let g:template['tex']['h5'] = "\\subparagraph{".g:rs."...".g:re."}"

let g:template['tex']['url'] = "\\url{".g:rs."...".g:re."}"

let g:template['tex']['list']  = "\\begin{itemize}\<cr>\\item ".g:rs."item text".g:re."\<cr>\\end{itemize}\<cr>"
let g:template['tex']['list2'] = "\\begin{enumerate}\<cr>\\item ".g:rs."item text".g:re."\<cr>\\end{enumerate}\<cr>"
let g:template['tex']['item']  = "\\item "

let g:template['tex']['table'] = "\\begin{tabular}{ l c r }\<cr>  & & \\\\ \<cr>\\end{tabular}\<cr>"

let g:template['tex']['pre'] = "\\begin{verbatim}\<cr>".g:rs."some data....".g:re."\<cr>\\end{verbatim}\<cr>"

let g:template['tex']['emph'] = "\\emph{".g:rs."...".g:re."}"
let g:template['tex']['b']    = "\\textbf{".g:rs."...".g:re."}"

let g:template['tex']['mark'] = "% MARK (\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>) "
let g:template['tex']['todo'] = "% TODO ".g:rs."some data....".g:re." (\<c-r>=strftime(\"%Y-%m-%d %H:%M:%S\")\<cr>) "

" Beamer
let g:template['tex']['frame'] = "\\begin{frame}{".g:rs."...".g:re."} \<cr>\\end{frame}"
...
let g:template['_']['date'] = "\<c-r>=strftime(\"%Y-%m-%d %H:%M\")\<cr>"
let g:template['_']['year'] = "\<c-r>=strftime(\"%Y\")\<cr>"
